...
const createFormMonth = (months) => {
    ...
    const buttonsMonth = months.map((item) => {
        ...
        btn.className = 'form__btn-mounth';
        btn.type = 'button';
        btn.textContent = `${item[0].toUpperCase()}${item.substring(1).toLowerCase()}`;

        if (item === month) {
            btn.classList.add('form__btn-mounth_active');
        }
    ...
    buttonsMonth.forEach(btn => {
        btn.addEventListener('click', ({ target }) => {
            if(target.classList.contains('form__btn-mounth_active')) {
                return
            }

            buttonsMonth.forEach (btn => {
                btn.classList.remove('form__btn-mounth_active')
            });

            target.classList.add('form__btn-mounth_active');
...
const init = () => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch('https://transparent-bittersweet-smelt.glitch.me/api/orders',
                {
                    method: 'POST',
                    headers: {
            ...
            if (response.ok) {
                alert('Данные успешно оптравленны');
                form.innerHTML = `<h2>Данные успешно отправленны</h2>`;
            } else {
                throw new Error(`Ошибка при отправки данных: ${response.status}`);
            }
